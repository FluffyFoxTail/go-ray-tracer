...
import (
	...
	"github.com/Fluffyfoxtail/go-ray-tracer/ray-tracer/primitives/materials"
)
...
func RandomScene() *primitives.World {
	...
	floor := primitives.NewSphere(0, -1000, 0, 1000,
		materials.Lambertian{Attenuation: primitives.Color{
			R: 0.5,
			G: 0.5,
			B: 0.5}})
			...
			if center.Subtract(primitives.Vector{X: 4, Y: 0.2, Z: 0}).Length() > 0.9 {
				if material < 0.8 {
					...
					metal := primitives.NewSphere(center.X, center.Y, center.Z, 0.2,
						materials.Metal{Attenuation: primitives.Color{
	...
	glass := primitives.NewSphere(0, 1, 0, 1.0, materials.Glass{Index: 1.5})
	lambertian := primitives.NewSphere(-4, 1, 0, 1.0, materials.Lambertian{Attenuation: primitives.Color{R: 0.4, G: 0.0, B: 0.1}})
	metal := primitives.NewSphere(4, 1, 0, 1.0, materials.Metal{Attenuation: primitives.Color{R: 0.7, G: 0.6, B: 0.5}, Fuzz: 0.0})
